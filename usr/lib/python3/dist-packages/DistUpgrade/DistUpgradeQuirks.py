...
class DistUpgradeQuirks(object):
    ...
    def noblePostInitialUpdate(self):
        ...
        self._set_grub_cloud_style_installation_on_cloud_installs()
        self._test_and_fail_on_unmerged_usr()
        self._test_and_fail_rabbitmq_server_installed()
    ...
    def _replaceDebsAndSnaps(self):
        ...
        for snap, snap_object in self._snap_list.items():
            ...
            if command == 'switch':
                ...
                if snap_object.get('switch_is_forced'):
                    # We are force switching the snap tracking. Highlight this
                    # to the user LP: #2077344.
                    self._view.information(
                        _(f'{snap} is now tracking {channel}'),
                        _(
        ...
        if snap_list:
            ...
            for snap in snaps_installed:
                if snap in seeded_snaps or snap in unseeded_snaps:
                    ...
                    seeded_snaps[snap] = (
                        None, from_channel, to_channel, False
                    )
    ...
    def _install_t64_replacement_packages(self):
        ...
        for package in self.controller.cache:
            ...
            for ver in provides_list:
                ...
                if ver.parent_pkg.architecture != package._pkg.architecture:
                    logging.debug(
                        f'Skip cross-grade replacement {ver.parent_pkg.name} '
                        f'for {package.name}'
                    )
                    continue
                ...
                if replacement.candidate is None:
                    continue
    ...
    def _set_grub_cloud_style_installation_on_cloud_installs(self):
        """
        LP: #2054103
        """

        if (
            os.access("/etc/cloud/build.info", os.R_OK) and
            "grub-pc" in self.controller.cache and
            self.controller.cache["grub-pc"].is_installed
        ):
            logging.debug('Detected cloud image with grub-pc installed')

            cmds = (
                'echo "grub-pc grub-efi/cloud_style_installation '
                'boolean true" | debconf-set-selections',
                'echo "grub-pc grub-pc/cloud_style_installation '
                'boolean true" | debconf-set-selections',
            )
            ...
            for cmd in cmds:
                ...
                if r.returncode != 0:
                    logging.debug(
                        f'Failed to run \'{cmd}\'. grub-pc package upgrades '
                        'will continue to not update the bootloader.'
                    )

    def _disable_stub_resolver(self):
        """
        LP: #2043820
        """
        ...
        # If systemd-resolved is not in use, we have nothing to do.
        r = subprocess.run(
            ['systemctl', 'is-active', '--quiet', 'systemd-resolved.service']
        )
        if r.returncode != 0:
            logging.debug(
                'systemd-resolved is not running, not disabling stub-resolver'
            )
            return

        logging.debug(
            'Disabling systemd-resolved stub resolver during upgrade'
        )

        os.makedirs('/run/systemd/resolved.conf.d', exist_ok=True)

        with open(
            '/run/systemd/resolved.conf.d/dist-upgrade-no-stub-resolver.conf',
            'w'
        ) as conf:
            conf.write('# Temporarily disable stub resolver during upgrade\n')
            conf.write('[Resolve]\n')
            conf.write('DNSStubListener=no\n')
        ...
        try:
            ...
            logging.debug(f'Failed to disable stub resolver: {e}')
    ...
    def _test_and_fail_on_unmerged_usr(self):
        ...
        for d in ('/bin', '/sbin', '/lib'):
            try:
                ...
                # This should not happen. Treat it like a failure just in case.
                fail = True
                break
    ...
    def _disable_cloud_init(self):
        ...
        Desktop metapackages now have Recommends: cloud-init, so it may be
        installed for the first time on upgrades to Noble. It will run on the
        first reboot after the upgrade by default, but we want to prevent that.
        """
        try:
            pkg = self.controller.cache['cloud-init']

            if not pkg.is_installed and pkg.marked_install:
                os.makedirs('/etc/cloud', exist_ok=True)
                with open('/etc/cloud/cloud-init.disabled', 'w') as f:
                    f.write(
    ...
    def _install_pipewire_audio_on_ubuntu_studio(self):
        ...
        if (
            ...
            logging.debug(
                'Detected Ubuntu Studio, installing pipewire-audio, '
                'removing pulseaudio'
            )
            ...
            try:
                ...
                for package_name in (
                    ...
                    self.controller.cache[package_name].mark_install(
                        auto_fix=False
                    )
                    apt.ProblemResolver(self.controller.cache).protect(
                        self.controller.cache[package_name]
                    )
                ...
                for package_name in (
                    ...
                    self.controller.cache[package_name].mark_delete(
                        auto_fix=False
                    )
                    apt.ProblemResolver(self.controller.cache).protect(
                        self.controller.cache[package_name]
